...
class Bockmayr_DataSet(Dataset):
    ...
    def __init__(self, mdlParams, indSet,train_idx = None):
        """
        Args:
        ...
        if self.orderedCrop and (indSet == 'valInd' or self.train_eval_state  == 'eval' or indSet == 'testInd'):
            ...
            # no Gray Scale (take color image)
    ...
    def __getitem__(self, idx):
        ...
        if self.indSet == 'trainInd':            
            return x, y, idx
            #return temp_new, y, int(train_idx),patient_path
        return x, y, idx

class Bockmayr_DataSet_New(Dataset):
    """Digital Pathology Data Set"""
        ...
        if self.orderedCrop and (self.indSet == 'valInd' or self.indSet == 'testInd' or self.train_eval_state == 'eval'):
            ...
            # training with patient based batchting
                ...
                for i in range(self.multiCropEval):
                        ...
                        x_new = x[:,(x_loc-np.int32(self.input_size[0]/2.)):(x_loc-np.int32(self.input_size[0]/2.))+self.input_size[0],(y_loc-np.int32(self.input_size[1]/2.)):(y_loc-np.int32(self.input_size[1]/2.))+self.input_size[1]]
                        #x = self.composed(x)
                        #x_new = self.composed(x_new)
                        #x_new = self.norm(x_new)
                        #print((x_loc-np.int32(self.input_size[0]/2.)),(x_loc-np.int32(self.input_size[0]/2.))+self.input_size[0])
                        #print((y_loc-np.int32(self.input_size[1]/2.)),(y_loc-np.int32(self.input_size[1]/2.))+self.input_size[1])
...
class ConcatMag(Dataset):
    ...
    def __getitem__(self,idx):
        x = torch.cat((self.data2000[idx][0],self.data4000[idx][0],self.data8000[idx][0]),dim = 1)
...
def getErrClassification(mdlParams, indices, modelVars):
    ...
    if 'multiCropEval' in mdlParams and mdlParams['multiCropEval'] > 0 and mdlParams.get('model_type_cnn') is None:
        ...
        for i, (inputs, labels, inds) in enumerate(modelVars['dataloader_'+indices]):
            ...
            with torch.set_grad_enabled(False):
                # Get outputs
                outputs = modelVars['model'](inputs.unsqueeze(0))#.unsqueeze(0)
                #print('a ',inputs.shape, outputs)
                preds = modelVars['softmax'](outputs)
                # Loss
                #print('b  ',inputs.shape, preds.shape, labels.shape)
                #print(labels)
                '''
                temp_label = []
                for i in range(mdlParams['multiCropEval']):
                    temp_label.append(labels[i*mdlParams['multiCropEval']])
                #temp_label.append()
                labels = torch.tensor(temp_label).to(modelVars['device'])
                print(labels.shape)
                '''
                #pritn()
                loss = modelVars['criterion'](outputs, labels[0].unsqueeze(0))
                #print("Loss Value One", loss)
            # Write into proper arrays
            loss_mc[i] = np.mean(loss.cpu().numpy())
            #print("Loss Value One: ", loss_mc)
            ...
            allInds[i] = inds.data.cpu().numpy()[0]
