import sys
sys.path.append('/home/Mukherjee/MProject/source_code/WSI_classifiers/utils')
import utils
...
class WholeSlideImage(object):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 95, mthresh=7, close = 0, use_otsu=False,
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            ...
            """
            filtered = []
            #print('*********params',seg_level, sthresh_up )
        ...
        #print(img2)
        cv2.imwrite('/cmpt/Mukherjee/Pant/file_analysis/'+self.name+ 'img.jpeg', img2)


        hsv_nemo = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
        #thresh = cv2.inRange(hsv_nemo, (0, 0, 0), (199,199, 110))  ##black works!!
        cv2.imwrite('/cmpt/Mukherjee/Pant/file_analysis/'+self.name+ 'hsv_nemo.jpeg', hsv_nemo)
        thresh = cv2.inRange(hsv_nemo, (69, 37, 0), (134,255, 255)) ##blue num_workers
        #thresh = cv2.inRange(hsv_nemo, (0, 45,40), (122, 255, 255)) ##red
        #(20, 100, 100), cvScalar(30, 255, 255
        #thresh = cv2.inRange(hsv_nemo, (10, 50,50), (40, 255, 255)) ##yellow
        #cv2.imwrite('file_analysis/'+self.name+ 'thre.jpeg', thresh)
        #cv2.imwrite('iorig.jpeg', img)
        ...
        for c in cnts:
            ...
            if area<555:
                ...
                #print(area)
                cv2.drawContours(thresh, [c], -1, (0,0,0), thickness=cv2.FILLED)
        #cv2.imwrite('file_analysis/'+self.name+ 'drawContours.jpeg', thresh)
        #thresh = cv2.erode(thresh,(3,3),iterations = 5)
        #cv2.imwrite('file_analysis/'+self.name+ 'erode.jpeg', thresh)
        thresh = cv2.dilate(thresh, (3,3), iterations=2)
        #cv2.imwrite('file_analysis/'+self.name+ 'dilate.jpeg', thresh)
        thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, (3,3),iterations =1)
        #cv2.imwrite('file_analysis/'+self.name+ 'close.jpeg', thresh)
        #cv2.imwrite('thres.jpeg', thresh)
        #cnts = cv2.findContours(image=thresh, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_NONE)
        cv2.imwrite('file_analysis/'+self.name+ 'thresh.jpeg', thresh)
        ...
        img = cv2.bitwise_or(img, thresh)
        #result = (gray/255.0) & (thresh/255.0)
        #thresh = cv2.threshold(thresh,127,255,cv2.THRESH_BINARY)
        #result = cv2.multiply(gray/255.0, thresh)
        #cv2.imwrite('foo.png', thresh)
        #cv2.imwrite('img_otsu.jpeg', result)
        ...
        #print('/*/*/****counter:', cv2.countNonZero(img_otsu))
        # Thresholding
        ...
        if use_otsu:
            ...
            #img_otsu = cv2.inRange(img_otsu, (0, 0, 0,0), (180, 255,0,0))
